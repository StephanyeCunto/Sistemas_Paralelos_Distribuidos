...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Imgcodecs.imwrite("saida_paralelizada_virtual.png", dst);
    ...
    private static void parallelBlur(Mat src, Mat dst, int numThreads) {
        ...
        for (int i = 0; i < numThreads; i++) {
           ...
           // Thread t = Thread.startVirtualThread(() -> processBlock(src, dst, width, yStart, yEnd));
            Thread t = Thread.ofVirtual().name("Thread Virtual(" + i + ")")
                 .start(() -> processBlock(src, dst, width, yStart, yEnd));
            threads.add(t);
        }
